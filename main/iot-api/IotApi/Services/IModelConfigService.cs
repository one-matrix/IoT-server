...
namespace IotApi.Services
{
    ...
    public interface IModelConfigService
    {
        ...
        Task<string> GetModelNameByIdAsync(string id);
        ...
        Task<AiModelConfig> GetModelByIdFromCacheAsync(string id);
        
        /// <summary>
        /// 设置默认模型
        /// </summary>
        /// <param name="modelType">模型类型</param>
        /// <param name="isDefault">是否默认（1:是，0:否）</param>
        Task SetDefaultModelAsync(string modelType, int isDefault);
        
        /// <summary>
        /// 获取符合条件的TTS平台列表
        /// </summary>
        /// <returns>TTS平台列表</returns>
        Task<IEnumerable<object>> GetTtsPlatformListAsync();
    }
}